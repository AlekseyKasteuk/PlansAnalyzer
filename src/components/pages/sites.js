...
class SitesView extends Component {
                        ...
                        backgroundColor: !this.props.privateSites.selected ? 'white' : this.props.privateSites.selected.id === site.id ? '#eff6fa' : 'white'
                                        ...
                                        <View style={ { flex: 1, paddingLeft: 10, paddingRight: 5 } }>
                                            <View><Text numberOfLines={1}
                                                        style={ styles.titleText }>{ site.title }</Text></View>
                                            <View><Text numberOfLines={1}
                                                        style={ styles.aditionalFieldsText }>{ site.address }</Text></View>
                                            {
                                                (() => {
                                                    if (site.description) {
                                                        return <View><Text numberOfLines={1}
                                                                           style={ styles.aditionalFieldsText }>{ site.description }</Text></View>
                                                    }
                                                })()
                                            }
                                        </View>
    ...
    drawSelectedSite() {
        ...
        if (!this.props.privateSites.selected) {
            return null
        }
        ...
        return (
            <View style={ { flex: 1 } }>
                <View
                    style={ { flexDirection: 'row', justifyContent: 'space-between', backgroundColor: '#ffe500', marginBottom: 20 } }>
                    <View style={ { width: 70, height: 40, padding: 5 } }>
                        <Image source={ { uri: this.props.privateSites.selected.image } }
                               style={ { width: 60, height: 60, borderRadius: 30 } }/>
                    </View>
                    <View style={ { flex: 1, justifyContent: 'center', alignItems: 'center', padding: 14 } }>
                        <Text style={{ fontSize: 15, fontWeight: 'bold' }}
                              numberOfLines={1}>{ this.props.privateSites.selected.title }</Text>
                    </View>
                    <View>
                        <TouchableWithoutFeedback>
        ...
        return (
            <View style={ [defaultPageStyles.pageWithStatusBar, { zIndex: 3 }] }>
              ...
              borderRightWidth: 1
                        ...
                        <View
                            style={{ flexDirection: 'row', justifyContent: 'space-between', borderBottomWidth: this.state.searchers.sites.visible ? 0 : 1, borderBottomColor: '#ccc' }}>
                            <View></View>
                            <View>
                                <TouchableWithoutFeedback
                                    onPress={ () => {
                        ...
                        borderWidth: 0
                                    ...
                                    >
                                        <Icon name="search"
                                              color={ this.state.searchers.sites.visible ? '#fff' : '#000' }
                                              style={{ fontSize: 20 }}
                                        />
                                    </View>
                                </TouchableWithoutFeedback>
                    ...
                    this.setState(newState);
                            ...
                            >
                                <TextInput ref={ 'seacrhSitesInput' } keyboardType="web-search"
                                           value={ this.state.searchers.sites.query }
                                           onChangeText={ (text) => { let newState = {...this.state}; newState.searchers.sites.query = text; this.setState(newState); } }
                                           style={{ height: 30, flex: 1, borderWidth: 1, borderColor: '#ccc', fontSize: 12, paddingTop: 3, paddingRight: 5,
                  ...
                  />
                    ...
                    <Animated.View
                        style={ { width: this.state.animations.centralPanel.width, zIndex: 2, overflow: 'hidden', borderRightColor: '#ccc', borderRightWidth: this.props.privateSites.selected ? 1 : 0 } }>
